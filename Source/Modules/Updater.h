...
// This is used for its type definitions and utility macros in this file.
#include <Declarations.h>
// This is needed to properly store key-value pairs in the renderer's keybuffer.
#include <Map.h>
// We use helper functions from this file to handle window-related control
// shortcuts.
#include <Window.h>

/**
 * @brief A structure to hold the data relevant to updating a window each frame.
 * Well, that description isn't entirely perfect, as the actual application data
 * (NPCs, sprites, player characteristics, etc.) are all stored in a different
 * object. This simply @b facilitates that work.
 */
typedef struct Updater
{
    ...
    u8 tick_speed;
    /**
     * @brief A buffer to hold each key that has been pressed in the last 100
     * milliseconds, each awaiting their turn to be pressed again. There are
     * only 21 items in this map, because only 21 keys do a function in-game.
     */
    Map* key_buffer;
} Updater;
...
/**
 * @brief Kill the updater. This frees all resources related to the object.
 ...
 */
void HandleInput(Updater* updater, Window* window, f32 delta_time, i32 key);
