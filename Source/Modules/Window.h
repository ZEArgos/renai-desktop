...
// Provides the functionality to log things to the standard output / error
// medium.
#include <Logger.h>

/**
 * @brief A wrapper for GLFW's @ref GLFWwindow with some added information for
 * ease of information.
 */
typedef struct Window
{
    /**
     * @brief The inner, wrapped GLFW window. This does the heavy lifting.
     */
    GLFWwindow* inner_window;
    /**
     * @brief The width of the window when it was first created, used for
     * restoration purposes.
     */
    i32 window_width;
    /**
     * @brief The height of the window when it was first created, used for
     * restoration purposes.
     */
    i32 window_height;
} Window;
...
/**
 * @brief An enum to describe the possible states of fullscreen that a window
 * can be in (windowed, maximized, or borderless).
 */
...
/**
 * @brief Create a new window, and set up some of the various under-the-hood
 * bits of it.
...
/**
 * @brief This is a simple getter to grab the inner window of a Window object.
 * It is basically just a replacement for @ref window->inner_window.
...
/**
 * @brief Get a flag telling the application if the given window should close or
 * not.
 * @param window The window to check.
 * @return A boolean expression, false for the window should continue, true for
...
/**
 * @brief Set the monitor of the given window, using its stored size and the
 * primary monitor.
 * @param window The window to set.
 */
__INLINE void SetWindowMonitor(Window* window)
{
    glfwSetWindowMonitor(window->inner_window, glfwGetPrimaryMonitor(), 0, 0,
                         window->window_width, window->window_height, -1);
}

/**
 * @brief Remove the monitor of the given window, sending it back to bordered
 * mode.
 * @param window The window to set.
 */
__INLINE void UnsetWindowMonitor(Window* window)
{
    glfwSetWindowMonitor(window->inner_window, NULL, 0, 0, window->window_width,
...
/**
 * @brief Similarly to @ref SetWindowFullscreenState, this function affects the
 * given window's fullscreen state, but instead of setting it the method just
...
/**
 * @brief Restore the window to a normal, bordered state of whatever size the
 * window was orginally set to.
 */
#define RestoreWindow(window) SetWindowFullscreenState(window, windowed)

/**
 * @brief Sister function to @ref RestoreWindow, toggles the states instead of
 * sets it.
 */
...
/**
 * @brief Sister function to @ref MaximizeWindow, toggles the states instead of
 * sets it.
 */
...
/**
 * @brief Sister function to @ref FullscreenWindow, toggles the states instead
 * of sets it.
 */
