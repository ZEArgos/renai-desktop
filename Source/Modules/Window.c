...
/**
 * @brief The callback triggered by GLFW when the framebuffer's (image
 * buffer) size changes.
 * @param window The window whose size changed. (We don't use this
 * variable.)
...
{
    // Figure out which side we're going to be clamping the aspect
    // ratio on.
    i32 smallest_dimension = fmin(width, height),
        lower_left_corner_x = 0, lower_left_corner_y = 0;
...
{
    Window* window = __MALLOC(
        Window, window,
        ("Failed to allocate the space for the key window. Code: %d.",
         errno));
    PrintSuccess("Allocated memory for the main window of the "
                 "application: %d bytes.",
                 sizeof(Window));
    ...
    // Create the window with the given width, height, a title of
    // whatever TITLE is defined as, bordered, and primary-monitored.
    window->inner_window =
        glfwCreateWindow(width, height, TITLE, NULL, NULL);
    if (GetInnerWindow(window) == NULL) PollGLFWErrors();
    PrintSuccess("Created the window with title '%s' successfully.",
                 TITLE);

    glfwMakeContextCurrent(GetInnerWindow(window));
    InitializeGLAD();

    // Call the new framebuffer callback as to set the scissor box
    // correctly.
                              ...
                              WindowFullscreenState state)
{
    ...
    {
        case windowed:
            // We unset the window monitor both here and on the
            // "maximized" case simply because I believe it provides a
            // more polished experience when we can switch between
            // states seamlessly; without this call the window
            // wouldn't become maximized/restored until borderless was
            // unset.
...
void ToggleWindowFullscreenState(Window* window,
                                 WindowFullscreenState state)
{
    // A little blocky, but it's far more efficient to do this as an
    // if chain than a switch.
    if (state == windowed || state == maximized)
        SetWindowFullscreenState(
            window, !glfwGetWindowAttrib(GetInnerWindow(window),
                                         GLFW_MAXIMIZED));
