...
{
    // Define the body expression of the switch statement. In this
    // case, we simply assign a value to the given member.
...
{
    // Define the body expression of this function's switch. Here, we
    // simply return the member requested.
#define READ(state, size) return TTVP(affected->__FIELD(state, size));
    ...
    // This part can't ever happen, but it's here to remove compiler
    // warnings.
    return NULL;
}
...
__BOOLEAN CompareAmbiguousType(AmbiguousType* affected,
                               ...
                               void* value)
{
    // Define the body expression for this particular function's
    // switch statement. Here, we compare the value given and the
    // value of the given member, and return the result.
#define COMPARE(state, size)                                         \
    ...
    // This part can never happen, but it's here to assuage compiler
    // concerns.
    return false;
}
